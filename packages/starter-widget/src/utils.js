...
export function getRandomString(len) {
  ...
  var randomString = "";
  for (var i = 0; i < len; i++) {
    randomString += possibleVal.charAt(
      Math.floor(Math.random() * possibleVal.length)
    );
  }
  return randomString;
}
...
export function getRandomCarDetails() {
  ...
  return {
    make: car[index]["make"],
    model:
      car[index]["models"][getRandomInt(0, car[index]["models"].length) - 1],
    license: license,
  };
}
...
export function getRandomLatLng() {
  return [
    (getRandomInt(0, 1800001) - 1) / 10000 - 90,
    (getRandomInt(0, 3600001) - 1) / 10000 - 180,
  ];
}
